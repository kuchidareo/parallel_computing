...
double median5(double values[5])
{
	for (int i = 0; i < 4; i++)
		for (int j = i + 1; j < 5; j++)
...
{      
	int i, j; 
	double start, naive_times[5], recursive_times[5];
...
//
	...
	for (int run = 0; run < 5; run++) {
		start = omp_get_wtime();
		matmult1(M, N, P, A, B, C1);
		naive_times[run] = omp_get_wtime() - start;

		start = omp_get_wtime();
		matmultr(M, N, P, A, B, C4);
		recursive_times[run] = omp_get_wtime() - start;
	}

	double time1 = median5(naive_times);
	double time2 = median5(recursive_times);

   printf("Matrix Dimensions: M = %d  P = %d  N = %d\n\n", M, P, N);
	printf("T naive median = %f seconds\n", time1);
	printf("T recursive median = %f seconds\n", time2);
   ...
   if (CheckResults(M, N, C1, C4))
     ...
     printf("OKAY\n\n");
     printf("T naive median / T recursive median = %f\n", time1/time2);
   }
